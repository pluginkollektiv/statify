			...
			<table>
				<?php foreach ( (array) $stats['target'] as $target ) { ?>
					<tr>
						...
						<td class="t">
						<a href="<?php echo esc_url( home_url( $target['url'] ) ); ?>" target="_blank" rel="noopener noreferrer" title="<?php echo 0 == url_to_postid( $target['url'] ) ? esc_html( get_bloginfo( 'name' ) ) : esc_html( get_the_title( url_to_postid( $target['url'] ) ) ); ?>">
